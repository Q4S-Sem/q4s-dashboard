    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {AUTOMATION_PRESETS.map((p, idx) => {
            ...
            return (
              <div key={p.name} className="flex flex-col rounded-xl border border-slate-200 bg-white p-4">
                <p className="text-sm font-semibold text-slate-900">{p.name}</p>
                <p className="mt-1 flex-1 text-xs text-slate-500">“{p.template}”</p>
                <div className="mt-3">
                  {already ? (
      ...
      <Card>
        <CardHeader>
          ...
          <span className="text-sm text-slate-400">{rules.length}</span>
        </CardHeader>
        {rules.length === 0 ? (
            ...
            {rules.map((r) => (
              <li key={r.id} className="flex flex-wrap items-center gap-3 px-5 py-3">
                <div className="min-w-0 flex-1">
                  <p className="flex flex-wrap items-center gap-2 font-medium text-slate-900">
                    ...
                    {r.trigger !== "INVOICE_OVERDUE" && (
                      <span className="text-xs font-normal text-slate-400">binnen {r.thresholdDays} dgn</span>
                    )}
                  </p>
                  ...
                  <button
                    ...
                    className={
                      r.active
                        ...
                        : "rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-500 hover:bg-slate-200"
                    }
                  >
                    {r.active ? "Actief" : "Uit"}
                  </button>
                ...
                <form action={deleteRule}>
                  <input type="hidden" name="id" value={r.id} />
                  <button type="submit" title="Verwijderen" aria-label="Regel verwijderen" className="text-slate-300 transition-colors hover:text-rose-600">
