...
export function CertPeopleList({ people }: { people: CertPerson[] }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {filtered.length === 0 ? (
        <p className="rounded-xl border border-dashed border-ink-300 bg-white px-4 py-8 text-center text-sm text-ink-500">
          {people.length === 0
        ...
        <div className="divide-y divide-ink-100 overflow-hidden rounded-xl border border-ink-200 bg-white">
          {filtered.map((p) => (
            <Link
              key={p.href}
              href={p.href}
              className="group flex items-center justify-between gap-3 px-4 py-3 transition-colors hover:bg-ink-50"
            >
              <div className="flex min-w-0 items-center gap-3">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-ink-100 text-ink-500 group-hover:bg-brand-100 group-hover:text-brand-600">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {p.tag && (
                      <span className="shrink-0 rounded bg-ink-100 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-ink-500">
                        {p.tag}
                      </span>
                    )}
                  </div>
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                <ChevronRight className="h-4 w-4 text-ink-300 group-hover:text-brand-500" />
              </div>
            </Link>
          ))}
        </div>
