...
export async function loadDealFormOptions(currentDealId?: string) {
  const [recruiters, stages, targets, clients, vacancies, vacatureDeals, contacts, currentId] = await Promise.all([
    getRecruiters(),
    getStages(),
    ...
    // Echte CRM-vacatures: open deals ZONDER kandidaat (= vacature-leads). Deze
    // koppel je aan een plaatsing zodat de vacature in de keuzelijst verschijnt.
    db.deal.findMany({
  ...
  return {
    ...
    // Vacature-deals als koppelbare vacatures (huidige deal zelf uitsluiten).
    vacatureDeals: vacatureDeals
      .filter((d) => d.id !== currentDealId)
      .map((d) => ({ id: d.id, label: d.company ? `${d.title} — ${d.company}` : d.title })),
