      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-3 border-b border-slate-100 px-4 py-3">
          <div className="flex-1">
            <div className="text-sm font-semibold text-slate-900">{formatWeekLabel(monday)}</div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs uppercase tracking-wide text-slate-400">
                  ...
                  {DAY_LABELS.map((d, di) => (
                    <th key={d} className={cn("px-2 py-2 text-center font-medium", di >= 5 && "bg-slate-50 text-slate-400")}>
                      {d}
                    </th>
                  ))}
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="border-t border-slate-50 hover:bg-slate-50">
                    <td className="whitespace-nowrap px-4 py-2.5">
                      <Link href={`/uren/${r.id}`} className="font-medium text-slate-900 hover:text-brand-700">
                        {r.consultant}
                      </Link>
                    </td>
                    <td className="whitespace-nowrap px-4 py-2.5 text-slate-600">{r.client}</td>
                    {r.days.map((h, di) => (
                      <td
                        key={di}
                        className={cn(
                    ...
                    <td className="px-2 py-2.5 text-right">
                      <Link
                        ...
                        className="inline-flex rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-brand-700"
                        title="Bewerken"
              ...
              <tfoot>
                <tr className="border-t border-slate-200 bg-slate-50/70 text-xs font-semibold text-slate-600">
                  ...
                  {dayTotals.map((h, di) => (
                    <td key={di} className={cn("px-2 py-2 text-center tabular-nums", di >= 5 && "bg-slate-100/60")}>
                      {h > 0 ? formatHours(h) : "·"}
                    </td>
                  ))}
              ...
              {missing.map((m, mi) => (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block truncate text-xs text-slate-500">{m.client}</span>
                  </span>
                  <Plus className="h-4 w-4 shrink-0 text-slate-300 transition-colors group-hover:text-brand-600" />
                </Link>
              ))}
            </div>
          </div>
